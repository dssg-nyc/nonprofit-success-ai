...
/**
 * Architect's maturity model (architect-design.md §Maturity Model).
 *
 * Unlike Scout's routing stub, this is not a stand-in for an LLM call — the
 * spec defines the model as a mechanical rubric ("Points-Primary with Targeted
 * Override + Flag"), so a deterministic implementation IS the real thing.
 * A future Claude call would enrich the charter/plan narrative (see
 * architectPlan.ts), not this scoring.
 */
...
function scoreGovernance(a: CsaScoredAnswers): MaturityScore {
  switch (a.q13_reporting_automation) {
    case 'mostly_automated': return 3;
    case 'semi_automated': return 2;
    default: return 1;
  }
}
...
function scoreTooling(a: CsaScoredAnswers): MaturityScore {
  ...
  // Reporting tool without a CRM isn't in the rubric — treated as Developing
  // (a real tool exists, but not the full stack the rubric's Established requires).
...
function scoreDecisionCulture(a: CsaScoredAnswers): MaturityScore {
  switch (a.q11_decision_empowerment) {
    case 'anyone_with_access': return 3;
    case 'leadership_managers': return 2;
    default: return 1;
  }
}

function scoreTeamCapacity(a: CsaScoredAnswers): MaturityScore {
  if (a.q15_staff_confidence === 'low_comfort' && a.q16_budget_speed === 'case_by_case') return 1;
  if (a.q15_staff_confidence === 'dedicated_staff' && a.q16_budget_speed === 'fast') return 3;
  return 2;
}
...
export function scoreAssessment(answers: CsaScoredAnswers, scoutBucket: ScoutBucket): MaturityResult {
  ...
  // Step 2 — weighted composite (DI ×2, Gov ×2, Tooling/DC/TC ×1, max 21)
  const points = 2 * di_score + 2 * gov_score + tooling_score + dc_score + tc_score;
  let compositeLevel = bandFromPoints(points);

  // Step 3 — override, Data Infrastructure only: a Foundational DI caps the
  // composite at Developing. You can't run a project on data that doesn't exist.
  ...
  // Step 4 — mandatory flags: DI or Governance at Foundational becomes a
  // required, named workstream — never generic "areas to improve" language.
  ...
  // Step 5 — remediation scoping: 2+ flags → the 90-day plan is remediation-only.
  const remediationOnly = flaggedDimensions.length >= 2;

  // Cross-check against Scout: a Foundational-maturity org holding an
  // ML/Predictive bucket is a redirect-before-chartering signal.
...
export const DIMENSION_LABELS: { key: keyof MaturityResult; label: string; weight: number }[] = [
